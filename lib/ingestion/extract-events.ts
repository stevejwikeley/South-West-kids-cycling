...
const REGIONS = ["devon", "cornwall", "somerset", "bristol", "both"] as const;
...
const KIND_DESCRIPTION =
  "\"training\" for a single club's own coaching or academy session (usually weekly, \"Go-Ride\" style, members or sign-up only, discipline should be \"training\"); \"race\" for everything else, including series rounds, open events, and cluster sessions (several clubs training together, a few times a year — a real event even though it isn't a race, discipline \"clusters\"). A club training session almost always names the club that runs it — put that club in organiser_name so a human can attach it in the queue.";
...
const ExtractedEventSchema = z.object({
  ...
  region: z.enum(REGIONS).nullable().describe("Infer from venue/address if it clearly indicates Devon, Cornwall, Somerset, or Bristol; otherwise null. \"both\" means specifically Devon and Cornwall (a legacy label) — use it only when the event serves those two, not Somerset or Bristol."),
...
function buildSystemPrompt(): string {
  ...
  return `You extract youth cycling event listings for South West Kids Cycling, a calendar covering ages 5-16 in Devon, Cornwall, Somerset & Bristol, England. Today's date is ${today}.
...
- Only extract youth/junior cycling events, club coaching sessions, or events that clearly include age-group categories for under-16s. Skip adult-only road racing, enduro, and anything outside Devon/Cornwall/Somerset/Bristol.
...
- Disciplines: cx (cyclocross), xc (cross country mountain biking), road, tri (triathlon), gravel (gravel racing), duathlon (run-bike-run), clusters (a cluster session — several clubs training together, a few times a year; kind should be "race", since this is a genuine event for the main calendar, not a single club's own coaching), training (a single club's own recurring coaching or academy session, "Go-Ride" style; kind should be "training"), other. Skip events with no cycling leg at all (a pure running race, swim event, etc.) — this calendar only covers cycling and cycling-adjacent multisport.
- kind: "training" for a single club's own coaching or academy session (usually weekly, "Go-Ride" style, members or sign-up only) — discipline should be "training". "race" for everything else, including series rounds and open events, as well as cluster sessions (several clubs training together, a few times a year) — only cluster sessions take discipline "clusters"; series rounds and open events keep whatever discipline they actually are. A club training session almost always names the club that runs it — put that club in organiser_name so a human can attach it in the queue.
