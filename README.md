...
**Known limitation — run Playwright with `--workers=1`.** The default parallel run can report false failures that are dev-server contention, not a real regression: Next compiles routes on first hit, and two workers hammering a cold route at once can time out. `npx playwright test --workers=1` passes reliably (51/51 per project — chromium and mobile — 102/102 total at the time of writing); if a parallel run shows red, re-run with `--workers=1` before assuming something broke.
