...
**Legacy shim for pre-`kind` training subscriptions** (`isLegacyClustersTrainingRequest` in `app/calendar.ics/route.ts`, mirrored in `app/embed/page.tsx`): naming `?discipline=clusters` explicitly used to be a way to build a training-only subscription, before `kind` existed and before `clusters` meant what it means today (a cluster session — several clubs training together, a few times a year — a real race-calendar event). Correcting that domain mistake would otherwise silently empty out every subscription built the old way: `discipline=clusters` alone now resolves to `kind=race` by default, which a legacy training subscriber never wanted. Since `kind` is new in this same change, its total absence is a reliable fingerprint of a pre-change URL — *provided* every URL either builder (`SubscribeSelector`, `EmbedBuilderPage`) generates from now on always carries an explicit `kind` whenever a discipline chip is selected, which they do (`kind=race` in the no-training case, `kind=all`/`kind=training` otherwise). So a request with `discipline=clusters` and no `kind` param at all is treated as that old training subscription: the discipline filter widens to match both `clusters` and `training` — covering both the current data state (club-training rows still carry `discipline: 'clusters'`, since migration `0025_reclassify_club_training_retry.sql` is deliberately unapplied) and the post-migration one (club training moves to `discipline: 'training'`) — and the `kind` filter is skipped entirely. A request that adds any `kind` (including `kind=race`) opts back out of the shim and gets the literal, current meaning of `clusters`. **Removing this shim is a judgement call, not a measurement**: `calendar_feed_hits`, `calendar_feed_subscribers` and `calendar_feed_stats` all exist in the database, but nothing in the codebase writes to them, so there is no way to tell when the last legacy subscription has actually stopped polling.
