...
A public calendar of youth cycling races and events (cyclocross, XC, road, triathlon, gravel, duathlon) across Devon, Cornwall, Somerset & Bristol, England, for ages 5–16. Built with Next.js and Supabase, deployed on Vercel.
...
lib/
  actions/                 Server Actions ("use server"), one file per feature
                             ...
                             event or a pending-queue item to a series — see "Event publishing
                             paths"; bookings.ts
  ...
  pending-fields.ts         Resolves "what is this pending row proposing for field X" across
                             both pending shapes (a change_request's diff over the live event
                             vs. an ingested row's own columns) — shared by the pending edit
                             panel and the pending-to-series conversion
...
1. **Manual** — an admin or organiser fills in the event form directly. This includes recurring events (e.g. weekly club training): the "add event" pages offer a "Repeating event" mode (`EventOrSeriesForm` → `SeriesForm`) that picks a set of weekdays and a required end date, then generates one `events` row per occurrence up front (`lib/actions/event-series.ts`, `lib/recurrence.ts`) — bounded, so there's no background job involved. A date can be skipped individually, and editing one occurrence's own fields detaches it from the series (`series_detached`) so a later series-wide edit never overwrites that customization. An existing one-off event can also be turned into a series after the fact — its edit page has a "Convert to a recurring event" link (`?from=<eventId>` on `/admin|organiser/series/new`) that prefills the new-series form from that event (`eventRowToSeriesPrefill()`, `lib/actions/parse-series-form.ts`) and replaces the original standalone event with the generated occurrences once saved. The same conversion is available one step earlier, from the pending queue: the edit panel on any pending item has a "Convert to a recurring event" action (`?fromPending=<pendingId>`) that saves the in-panel edits, prefills the series form from the row's resolved values (`pendingRowToSeriesPrefill()`), and on save clears the item from the queue — replacing the live event it was matched to, if it had one, so the series occurrence for that date isn't duplicated. That path is admin-only in three places: the `/admin` layout, an `isAdminRole()` check in `saveSeries()`, and RLS on `events_pending`.
...
`/subscribe` is a feed builder, not just a set of instructions. `components/SubscribeSelector.tsx` offers the same three filter axes the feed itself understands — discipline (multi-select), county, and club — plus a "training only" toggle, composes them into a `/calendar.ics?discipline=…&region=…&club=…` URL (with `&kind=all`/`&kind=training` appended only once training is actually included — the default, no-training case never carries `kind` at all, races being the default), and feeds that URL into the per-platform (Google/Apple/Outlook) setup steps, so the link someone copies is already scoped to what they picked. Leaving everything untouched yields the plain full-calendar URL, exactly as before.

**`kind` (`app/calendar.ics/route.ts`, `events.kind`/`events_pending.kind`/`event_series.kind`):** every event is a race or a training session (`event_kind` enum), and the default feed — no `?kind` at all — is races only, matching the public calendar. Training reaches a subscriber two ways: `?kind=training` (that club/region/discipline's training only) or `?kind=all` (races and training together). A per-club training feed is `?club=<uuid>&kind=training`; `ClubTrainingBand` (`components/clubs/ClubTrainingBand.tsx`) on `/clubs` links directly to this shape for each club.

**Breaking change: `?discipline=clusters` no longer implies training.** An earlier version of this feed briefly shipped a shim that treated a bare `?discipline=clusters` (no `kind` at all) as a pre-`kind` training subscription, because `clusters` used to be mislabelled "Training session" and any URL built that way predates `kind` entirely. That shim was removed as a deliberate, consciously-accepted breaking change: it matched on `discipline in ('clusters', 'training')`, but training rows can legitimately carry any riding discipline now (5 of the 62 live training rows already carry `discipline='xc'`, not `'training'`), so the shim was already silently degrading — delivering a shrinking fraction of a subscriber's training rather than all of it. `discipline=clusters` now always means exactly what it says (a cluster session — several clubs training together, a few times a year, a genuine calendar event), with no special case for the absence of `kind`. Anyone holding a subscription built before the `clusters` taxonomy was corrected will stop receiving club training through it; they need a fresh `?kind=training` or `?kind=all` link from `/subscribe`.
...
**Known limitation — run Playwright with `--workers=1`.** The default parallel run can report false failures that are dev-server contention, not a real regression: Next compiles routes on first hit, and two workers hammering a cold route at once can time out. `npx playwright test --workers=1` passes reliably (58/58 per project — chromium and mobile — 116/116 total at the time of writing); if a parallel run shows red, re-run with `--workers=1` before assuming something broke.

`npm run test:unit` runs `lib/training.test.ts` (and any other `*.test.ts` under `lib/`) via Node's built-in test runner (`node --test --experimental-strip-types`) — no Playwright, no dev server, no Supabase. `lib/training.ts`'s derivation functions (`weekdayPattern`, `nextDistinctDates`, `commonVenue`, `distinctDates`, `groupByClub`, `sessionsInNextDays`) are covered here rather than in `e2e/` since they're pure and don't need a browser.
...
- **Feed & embed instrumentation**: neither the `.ics` feed nor `/embed` can be measured by a client-side script — a calendar app fetches `.ics` directly with no browser involved, and ad blockers plus many club sites' CSPs strip third-party analytics from an iframe. `lib/feed-hits.ts` logs every hit server-side instead (`after()`, so it never adds latency), into `calendar_feed_hits`. Query `calendar_feed_stats` for an estimated subscriber count and split by calendar client (a fingerprint only counts once it's fetched on 2+ distinct days, which is what separates a standing subscription from someone opening the link once in a browser), and `calendar_embed_referrers` for which sites have actually installed the widget, grouped by hostname from the `Referer` header. See [`supabase/migrations/0026_feed_hit_logging.sql`](supabase/migrations/0026_feed_hit_logging.sql).
