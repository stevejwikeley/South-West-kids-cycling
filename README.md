...
Earlier, naming `?discipline=clusters` explicitly was also a way in — a deliberate carve-out so training-only subscriptions people already held (from before `kind` existed) kept delivering rather than silently going empty. That carve-out has since been removed: `discipline: 'clusters'` now means an actual cluster session again, not club training, so it no longer implies training. **Consequence:** anyone who set up a `?discipline=clusters` subscription before this change now gets zero training events from it — that subscription needs to be replaced with `?kind=training` (or `?kind=all`) to keep receiving training.
